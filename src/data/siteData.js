...
export const majorProjectsData = [
  ...
  { id: 'h', title: '3D Fruit Ninja Simulation', cardDescription: 'A 3D \'Fruit Ninja\' built in MuJoCo with Python — physics-based modelling and 3D graphics for an interactive game world. A Foundations of Robotics project at NYU.', imageUrl: baseProjectImagePath + 'fruitninja.mp4', tags: ['Python', 'MuJoCo'], status: 'In Progress', modalContent: [{ type: 'text', value: "Created a 3D simulation of the game 'Fruit Ninja' using Python, applying principles of physics-based modeling and 3D graphics to develop an interactive and engaging virtual experience." }, { type: 'button', text: 'View on GitHub', link: 'https://github.com/AadhavSivakumar/MujocoSim' }] },
];

export const smallProjectsData = [
  { id: 7, title: 'Sluice — on-device drone surveillance', cardDescription: 'Dell × NVIDIA hackathon build on a single GB10 box: a drone flies a 6 km Amazon mining landscape in Isaac Sim while an RF-DETR-Seg model trained on real satellite imagery scores its camera live, and a sandboxed local agent turns geo-tagged captures into findings. Sim, detector, VLM, agent and database all run on one machine — no cloud API.', imageUrl: baseProjectImagePath + 'sluice.webp', tags: ['RF-DETR-Seg', 'Isaac Sim', 'vLLM', 'NVIDIA GB10', 'Agents'], status: 'Completed', modalContent: [{ type: 'text', value: 'Built at the Dell × NVIDIA hackathon (NYC, August 2026) on a Dell Pro Max with GB10 — Grace Blackwell, aarch64, 121 GB of unified memory. A quadrotor flies a 6 km illegal-mining landscape in NVIDIA Isaac Sim; its downward camera is scored live by an RF-DETR-Seg model trained on real satellite imagery of mining sites, and every frame is stored geo-tagged in MongoDB.' }, { type: 'text', value: 'An OpenClaw agent, sandboxed by OpenShell and wired to local inference through NVIDIA NemoClaw, reasons over the captures and writes findings and reports — with Qwen3-VL-30B (FP8, vLLM) or Nemotron 3.5 as the model, all on loopback. A unified console ties flight, detector and agent into one screen. A side demo drives a Franka FR3 in MuJoCo from a bare hand via webcam and MediaPipe.' }, { type: 'button', text: 'View on GitHub', link: 'https://github.com/AadhavSivakumar/Dell-x-NVIDIA-Hackathon-Sluice' }] },
